...
export default function (payload) {
    payload.on('pull_request', (body) => {
        ...
        (async function () {
            ...
            var changedFiles = await new Promise((resolve, reject) => {
                request
            ...
            var changedFilesAuthors = await new Promise((resolve, reject) => {
                execFile(
            ...
            for (let author of concernedAuthors) {
                if (!contribution[author]) contribution[author] = 0
                contribution[author]++
            }
        ...
        })().catch((error) => payload.emit('error', error))
    })
}
